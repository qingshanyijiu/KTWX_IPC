	...
	, maxBackupIndex(maxBackupIndex_)
{
	...
	//************************zg 2015.07.20**************************
	filename = getFilename(Time::gettimeofday());
...
DailyRollingFileAppender::init(DailyRollingFileSchedule sch)
{
    ...
    scheduledFilename = getscheduledFilename(now);
...
/*
	...
	*/
    ...
    {
	    ret = file_rename (scheduledFilename,filename);
...
log4cplus::tstring DailyRollingFileAppender::getscheduledFilename(const log4cplus::helpers::Time& t) const
{
	log4cplus::helpers::Time timeSche = t;
	tchar const * pattern = 0;
	switch (schedule)
	{
	case MONTHLY:
	...
	case DAILY:
		timeSche = t -Time(maxBackupIndex*24*60*60);
		pattern = LOG4CPLUS_TEXT("%Y-%m-%d");
		break;
	...
	tstring result (filename);
	result += LOG4CPLUS_TEXT(".");
	result += timeSche.getFormattedTime(pattern, false);
	return result;
}
